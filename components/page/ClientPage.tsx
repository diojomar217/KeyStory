...
const PayMongoButton = dynamic(() => import("../ui/PayMongoButton"), {
  ssr: false,
});
...
import BaptismHomeSection from "./BaptismHomeSection";
...
export default function ClientPage({
  ...
  const effectiveStatus = useMemo(() => {
    ...
    const expiresAt =
      config?.expires_at || config?.expiry_date || config?.expiration;
  ...
  const storySections = useMemo(
    () => activeSections.filter((section) => section !== "home"),
    [activeSections],
  );
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const nextSection =
          topVisible?.target?.getAttribute("data-section-key");
        if (nextSection) {
          setActiveStorySection(nextSection);
          trackAnalyticsEvent(
            "section_view",
            `section:${nextSection}`,
            `section_view_${nextSection}`,
          );
        }
      },
      {
  ...
  useEffect(() => {
    ...
    const storedUnlocked =
      typeof window !== "undefined"
        ? window.localStorage.getItem(`unlocked_${slug}`)
        : null;
    const storedHash =
      typeof window !== "undefined"
        ? window.localStorage.getItem(`unlocked_hash_${slug}`)
        : null;
  ...
  const { effectiveGalleryLayout, mergedTimelineEvents } = useMemo(() => {
    ...
    const mergedEvents = convertToTimelineEvents(
      timelineEvents,
      sectionContent,
    );
  ...
  const shouldShowTimeline =
    hasTimeline || hasFirstDate || hasSpecialMoments || hasMilestones;
  ...
  const sectionDividerStyle =
    (config?.section_divider_style as
      | "none"
      | "standard"
      | "gradient"
      | "dots") || "standard";
  ...
  const renderSectionSeparator = (
    ...
    if (sectionDividerStyle === "gradient")
      return <GradientSeparator key={key || "section-sep"} theme={theme} />;
    if (sectionDividerStyle === "dots")
      return <DotsSeparator key={key || "section-sep"} theme={theme} />;
    return (
      <SectionSeparator
        key={key || "section-sep"}
        theme={theme}
...
console.log('activeSections:', activeSections);
      switch (resolvedSection) {
        ...
        case "playlist":
          ...
          // 🔥 MAP DB TEMPLATE → COMPONENT TEMPLATE
          const dbTemplate =
            config?.templates?.playlist || // 🔥 MAIN SOURCE (DB)
            contentForSection?.playlistTemplate ||
            sectionContent?.playlist?.playlistTemplate;

          const mapTemplate = (
            tpl?: string,
          ): "minimal" | "visual" | "lyrics" => {
            switch (tpl) {
              case "minimal_player":
                return "minimal";
              case "visual_player":
          ...
          const resolvedSongLink =
            contentForSection?.playlistUrl ||
            sectionContent?.playlist?.playlistUrl ||
            config?.media?.song_link;

          if (!resolvedSongLink) {
            const title =
              contentForSection?.title ||
              sectionContent?.playlist?.title ||
              "Our Playlist";
            const subtitle =
              contentForSection?.subtitle ||
              sectionContent?.playlist?.subtitle ||
              "No song or playlist configured";

            return (
              <section
                key={section}
                className={`py-10 ${
                  variant === "alt" ? styles.sectionBgAlt : styles.sectionBg
                }`}
              >
                <div className="max-w-4xl mx-auto px-4 md:px-6">
                  ...
                  <p className={`mt-4 text-sm ${styles.textMuted}`}>
                    This playlist section is enabled but no song or playlist URL
                    was configured.
                  </p>
                </div>
              </section>
          ...
          return (
            <PlaylistSection
              ...
              siteType={siteType}
              autoplay={
                contentForSection?.song_autoplay ||
                sectionContent?.playlist?.song_autoplay ||
                config?.media?.song_autoplay
              }
              songLink={resolvedSongLink}
              playlistTemplate={mapTemplate(dbTemplate)} // 🔥 FINAL VALUE
              title={
                contentForSection?.title || sectionContent?.playlist?.title
              }
              subtitle={
                contentForSection?.subtitle ||
                sectionContent?.playlist?.subtitle
              ...
              songTitle={
                contentForSection?.songTitle ||
                sectionContent?.playlist?.songTitle
              }
              artistName={
                contentForSection?.artistName ||
                sectionContent?.playlist?.artistName
              }
              coverImageUrl={
                contentForSection?.coverImageUrl ||
                sectionContent?.playlist?.coverImageUrl
              }
              description={
                contentForSection?.description ||
                sectionContent?.playlist?.description
              }
              lyricsExcerpt={
                contentForSection?.lyricsExcerpt ||
                sectionContent?.playlist?.lyricsExcerpt
              }
            />
          );
        case "video_memories":
          ...
          return (
            <VideoMemoriesSection
              ...
              theme={theme}
              videos={
                contentForSection?.videos ||
                sectionContent?.video_memories?.videos
              }
            />
          );
        ...
        case "birthday_message":
          ...
          return (
            <BirthdayMessageSection
              ...
              theme={theme}
              message={
                (sectionContent as any)?.birthday_message?.content ||
                sectionContent?.birthday_message?.text ||
                message
              }
            />
          );
        ...
        case "birthday_wishes":
          ...
          return (
            <BirthdayWishesSection
              key="birthday_wishes"
              theme={theme}
              wishes={(sectionContent?.birthday_wishes?.quotes || [])
        ...
        case "gift_wishlist":
          return (
            <GiftWishlistSection
              ...
              theme={theme}
              items={
                contentForSection?.items || sectionContent?.gift_wishlist?.items
              }
            />
          );
        ...
        case "future_plans":
          return (
            <FutureDreamsSection
              ...
              siteType={siteType}
              dreams={
                contentForSection?.dreams ||
                sectionContent?.future_dreams?.dreams
              }
              variant={variant}
            />
          );
        ...
        case "tributes":
          return (
            <QuotesSection
              ...
              siteType={siteType}
              quotes={
                contentForSection?.quotes || sectionContent?.quotes?.quotes
              }
              variant={variant}
            />
          );
        ...
        case "message_letter":
          return (
            <LoveLetterSection
              key={section}
              message={
                contentForSection?.content || contentForSection?.text || message
              }
              theme={theme}
              siteType={siteType}
            />
        ...
        case "guest_messages":
          return (
            <GuestMessagesSection
              ...
              siteType={siteType}
              messages={
                contentForSection?.messages ||
                sectionContent?.guest_messages?.messages
              }
          ...
          return (
            <>
              <RSVPSection
                  ...
                  siteId={siteId || (config as any)?.siteId || undefined}
                />

              {/* Guest messages are only rendered when the dedicated
                  `guest_messages` section is present. Remove RSVP fallback. */}
            </>
          );
        ...
        case "invitation":
          return (
            <InvitationSection
              ...
              guestName={guestName}
              invitation={
                contentForSection || sectionContent?.invitation || undefined
              }
              invitationMessage={
                contentForSection?.invitationMessage ||
                sectionContent?.invitation?.invitationMessage ||
                message
              }
              godparentMessage={
                contentForSection?.godparentMessage ||
                sectionContent?.invitation?.godparentMessage ||
        ...
        case "memory_map":
          return (
            <MemoryMapSection
              ...
              siteType={siteType}
              locations={
                contentForSection?.locations ||
                sectionContent?.memory_map?.locations
              }
            />
          );
        ...
        case "letter_future":
          return (
            <LetterToFutureSection
              ...
              partnerName={resolvedPartnerName}
              letter={
                contentForSection?.letter ||
                sectionContent?.letter_future?.letter
              }
              openDate={
                contentForSection?.openDate ||
                sectionContent?.letter_future?.openDate
              }
            />
          );
        ...
        case "gift_section":
          return (
            <GiftSection
              ...
              partnerName={resolvedPartnerName || resolvedCustomerName}
              gifts={
                contentForSection?.gifts || sectionContent?.gift_section?.gifts
              }
            />
          );
        ...
        case "surprise_message":
          return (
            <SurpriseMessageSection
              ...
              partnerName={resolvedPartnerName}
              message={
                contentForSection?.message ||
                sectionContent?.surprise_message?.message
              }
              hint={
                contentForSection?.hint ||
                sectionContent?.surprise_message?.hint
              }
            />
          );
    ...
    const remainingSections = activeSections.filter(
      (section) => section !== "home",
    );
    const navSections =
      visibleStorySections.length > 0 ? visibleStorySections : storySections;
            ...
            <div className="relative z-10">
              ...
              {navSections.length > 0 && (
                <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[70] md:hidden">
                  {/* <div
                    ...
                    style={{
                      backgroundColor: checkIsDarkTheme(theme)
                        ? "rgba(17,17,17,0.62)"
                        : "rgba(255,255,255,0.78)",
                      borderColor: checkIsDarkTheme(theme)
              ...
              {(() => {
                ...
                remainingSections.forEach((section) => {
                  ...
                  const sectionAnimationDelay = Math.min(
                    0.25 + renderedSectionCount * 0.08,
                    0.9,
                  );
                  renderedNodes.push(
                    <div
              ...
              {(() => {
                const isQrKeepsakeEnabled =
                  Array.isArray(sections) &&
                  activeSections.includes("qr_keepsake");
    ...
    return (
      <>
        ...
        {showBanner && !isRevealing && (
          <div
            className={`w-full flex justify-center animate-fade-in-up motion-reduce:animate-none`}
          >
            <div
              className={`mt-4 mb-2 px-5 py-2 rounded-xl border text-sm font-semibold shadow-sm ${bannerColor} max-w-xl text-center`}
            >
              {bannerText}
            </div>
          </div>
  ...
  return (
    <>
      ...
      {showBanner && (
        <div
          className={`w-full flex flex-col items-center animate-fade-in-up motion-reduce:animate-none`}
        >
          <div
            className={`mt-4 mb-2 px-5 py-2 rounded-xl border text-sm font-semibold shadow-sm ${bannerColor} max-w-xl text-center`}
          >
          ...
          <PayMongoButton
            ...
            customerName={resolvedCustomerName}
            customerEmail={
              config?.people?.email || config?.customer_email || ""
            }
            className="mt-2"
            flowType="extension"
            slug={slug}
