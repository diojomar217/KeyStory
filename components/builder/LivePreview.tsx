...
import {
  getOccasionDesignSummary,
  getOccasionHeroSpec,
} from "@/config/occasionHeroConfig";
...
export default function LivePreview({
  ...
  const themePreset =
    THEME_CONFIG[themeKey as ThemeKey] || THEME_CONFIG[DEFAULT_THEME];
      ...
      <div className="hidden lg:block w-80 flex-shrink-0">
        <div className="sticky top-4">
          <div className="bg-white rounded-2xl shadow-xl border overflow-hidden">
            ...
            {/* Preview Area */}
            <div className="h-[520px]">{content}</div>
          </div>

          <div className="mb-3 mt-3 rounded-xl border border-slate-200/70 bg-white/80 p-3 shadow-sm backdrop-blur-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] ${occasionHero.renderStrategy === "dedicated" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"}`}
              >
                {occasionHero.renderStrategy === "dedicated"
                  ? "Dedicated"
                  : "Shared"}
              </span>
            </div>
      ...
      {isMobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[85vh] overflow-hidden animate-slide-up">
            ...
            {/* Content */}
            <div className="h-[calc(85vh-60px)]">{content}</div>
          </div>
        </div>
      )}
    </>
